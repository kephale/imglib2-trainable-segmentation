...
public class SingleHessianFeature extends AbstractFeatureOp {
	...
	private void calculateHessianOnChannel(FeatureInput image, RandomAccessibleInterval<FloatType> out, double sigma) {
		...
		LoopBuilder.setImages(Views.collapseReal(out), dxx, dxy, dyy).forEachPixel( (o, s_xx, s_xy, s_yy) -> {
			calculateHessianPerPixel(o, s_xx.get(), s_xy.get(), s_yy.get());
		});
	}

	private static void calculateHessianPerPixel(RealComposite<FloatType> output, double s_xx, double s_xy, double s_yy)
	{
		...
		output.get(EIGENVALUE_1).setReal(trace/2.0 + Math.sqrt((4*s_xy*s_xy + (s_xx - s_yy) * (s_xx - s_yy)) / 2.0 ));
		// Second eigenvalue: (a + d) / 2 - sqrt( ( 4*b^2 + (a - d)^2) / 2 )
		output.get(EIGENVALUE_2).setReal(trace/2.0 - Math.sqrt((4*s_xy*s_xy + (s_xx - s_yy) * (s_xx - s_yy)) / 2.0 ));
		// Orientation
		double orientation = ((s_xy < 0.0) ? - 0.5 : 0.5) * Math.acos((s_xx - s_yy) / Math.sqrt(4.0 * s_xy * s_xy + (s_xx - s_yy) * (s_xx - s_yy)));
		if (Double.isNaN(orientation))
			orientation = 0;
		output.get(ORIENTATION).setReal(orientation);
		// Gamma-normalized square eigenvalue difference
		output.get(SQUARE_EIGENVALUE_DIFFERENCE).setReal(Math.pow(t,4) * trace*trace * ( (s_xx - s_yy) * (s_xx - s_yy) + 4*s_xy*s_xy ));
		// Square of Gamma-normalized eigenvalue difference
		output.get(NORMALIZED_EIGENVALUE_DIFFERENCE).setReal(Math.pow(t,2) * ( (s_xx - s_yy) * (s_xx - s_yy) + 4*s_xy*s_xy ));
	}
}
