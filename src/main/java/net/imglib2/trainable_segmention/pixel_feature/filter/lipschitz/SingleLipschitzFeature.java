...
public class SingleLipschitzFeature extends AbstractFeatureOp {
	...
	private void apply(FeatureInput in, RandomAccessibleInterval<FloatType> out) {
		...
		copy(original, tmp);
		ConeMorphology.performConeOperation(ConeMorphology.Operation.DILATION, tmp, scaledSlope(in.pixelSize()));
	...
	private long[] scaledBorder(double[] pixelSizes, RandomAccessibleInterval<FloatType> out) {
		return Arrays.stream(pixelSizes).mapToLong(pixelSize -> (long) Math.ceil(border / pixelSize)).toArray();
	}

	private <T extends Type<T>> void copy(RandomAccessible<T> in, RandomAccessibleInterval<T> out) {
		LoopBuilder.setImages(Views.interval(in, out), out).multiThreaded().forEachPixel( (i, o) -> o.set(i) );
	}
	...
	private <T extends RealType<T>> void outEquals255PlusAMinusB(RandomAccessibleInterval<T> out, RandomAccessible<T> A, RandomAccessible<T> B) {
		...
		offset.setReal(255);
		LoopBuilder.setImages( Views.interval(A, out), Views.interval(B, out), out).forEachPixel( (a, b, o) -> {
			o.set(offset);
			o.sub(b);
			o.add(a);
		});
