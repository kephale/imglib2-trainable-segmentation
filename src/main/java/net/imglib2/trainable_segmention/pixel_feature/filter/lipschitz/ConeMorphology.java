...
class ConeMorphology {
	...
	private static Iterable<Localizable> neighborhood(int n) {
		...
		RandomAccess<Neighborhood<Localizable>> ra = shape.neighborhoodsRandomAccessible(Localizables.randomAccessible(n)).randomAccess();
		ra.setPosition(RevampUtils.nCopies(ra.numDimensions(), 0L));
		return ra.get();
	}

	private static <T extends RealType<T>> void oneRun(double[] slope, final long[] translation, final RandomAccessibleInterval<T> image, Operation operation) {
		...
		LoopBuilder.setImages(invertDirections(centers, translation), invertDirections(neighbors, translation))
				.flatIterationOrder()
	...
	private static <T extends RealType<T>> IntervalView<T> cutBorders(long[] translation, RandomAccessibleInterval<T> image) {
		return Views.interval(image, Intervals.intersect(image, Intervals.translate(image, translation)));
	}
	...
	private static <T extends RealType<T>> BiConsumer<T,T> getAction(Operation operation, double combinedSlope) {
		switch (operation) {
			case DILATION: return (center, neighbor) -> center.setReal( Math.max(center.getRealDouble(), neighbor.getRealDouble() - combinedSlope) );
			case EROSION: return (center, neighbor) -> center.setReal( Math.min(center.getRealDouble(), neighbor.getRealDouble() + combinedSlope) );
		}
