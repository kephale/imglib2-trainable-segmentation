...
public class FeatureInputTest {
	...
	private void testDerivativeKernel(DoubleUnaryOperator input, int order, double expected) {
		...
		RandomAccessibleInterval<DoubleType> image = Views.translate(ArrayImgs.doubles( inputValues, 5 ), -2);
	...
	private RandomAccessibleInterval<DoubleType> constant(double v, Interval interval) {
		return Views.interval(ConstantUtils.constantRandomAccessible(new DoubleType(v), interval.numDimensions()), interval);
	}
}
