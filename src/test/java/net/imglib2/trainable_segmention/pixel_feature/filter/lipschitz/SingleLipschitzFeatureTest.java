...
public class SingleLipschitzFeatureTest {
	...
	public void test() {
		...
		FeatureOp feature = SingleFeatures.lipschitz(0.1, 0).newInstance(new Context().service(OpService.class), GlobalSettings.default3d().build());
		...
		Img<FloatType> expected = createImage((x, y) -> (x == 2) && (y == 2) ? 0 : -1 + 0.1 * Math.sqrt( Math.pow(x - 2, 2) + Math.pow(y - 2, 2)));
	...
	public void testOpening() {
		Img<FloatType> image = dirac();
		Img<FloatType> expected = createImage((x, y) -> 1 - Math.sqrt( Math.pow(0.1 * (x - 2), 2) + Math.pow( 0.2 * (y - 2), 2)));
		ConeMorphology.performConeOperation(ConeMorphology.Operation.DILATION, image, new double[]{ 0.1, 0.2 });
	...
	public void testClosing() {
		...
		Img<FloatType> expected = createImage((x, y) -> - 1 + Math.sqrt( Math.pow(0.1 * (x - 2), 2) + Math.pow( 0.2 * (y - 2), 2)));
		ConeMorphology.performConeOperation(ConeMorphology.Operation.EROSION, image, new double[]{ 0.1, 0.2 });
		Utils.assertImagesEqual(35, expected, image);
	}
